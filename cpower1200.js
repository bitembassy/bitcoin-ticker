...
serialPort.list(function (err, ports)
{
 ports.forEach(function(port)
 {
  ...
  if((port.pnpId && port.pnpId.indexOf('Prolific')!=-1) || (port.manufacturer && port.manufacturer.indexOf('Prolific')!=-1))
  {
   ...
   myserial.on('error', function(a){ try{ console.log('serial error',a.stack); exports.serial=null; myserial=null; }catch(err){ if(err)console.log('err ', err.stack);} })
...
{
 try{
   if(exports.serial&&require('os').arch()=='arm'&& !fs.existsSync('/dev/serial/by-id/'+myserial.pnpId))
   {
